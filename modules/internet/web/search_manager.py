...
class SearchManager:
    ...
    def execute(
        ...
        *,
        ...
        used, retries_delta, errors_delta = self._run_query_batch(
            candidates=scout_candidates,
            query_limit=scout_limit,
            source_limit=policy.max_sources,
            collected=collected,
            seen_urls=seen_urls,
        ...
        fetched_pages = self._fetch_pages(
            rows=collected[: max(1, policy.max_fetches)],
            max_fetches=policy.max_fetches,
        )
        self._topic_last_ts[topic_key] = now

        return SearchExecutionResult(
            results=collected[: max(1, policy.max_sources)],
    ...
    def _run_query_batch(
        ...
        *,
        ...
        return (used, retries_used, errors_count)
    ...
    def _fetch_pages(self, *, rows: list[SearchResult], max_fetches: int) -> dict[str, dict[str, Any]]:
