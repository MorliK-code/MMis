...
def _settings_from_payload(payload: dict[str, Any], *, config_file: Path) -> AppSettings:
    ...
    settings = AppSettings(
        ...
        memory_core_enabled=_to_bool(_pick_value(_get_dotted(row, "memory_core.enabled"), True)),
        memory_core_db_path=_norm_str(_get_dotted(row, "memory_core.db_path") or "data/memory_core/memory.db"),
        memory_core_vector_path=_norm_str(_get_dotted(row, "memory_core.vector_path") or "data/memory_core/vector"),
        memory_core_default_workspace=_norm_str(_get_dotted(row, "memory_core.default_workspace") or "global"),
        memory_core_default_namespace=_norm_str(_get_dotted(row, "memory_core.default_namespace") or "default"),
        memory_core_top_k=max(1, _to_int(_get_dotted(row, "memory_core.top_k"), default=8)),
        memory_core_enable_background_worker=_to_bool(
            _pick_value(
                _get_dotted(row, "memory_core.enable_background_worker"),
                _get_dotted(row, "memory_core.worker_enabled"),
                True,
            )
        ),
        memory_core_worker_poll_interval=max(
            0.2,
