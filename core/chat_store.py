...
class ChatStore:
    ...
    def create_chat(self, title: str, persona_id: str = "default", chat_id: str | None = None) -> Chat:
        now = time.time()
        cid = str(chat_id or f"chat_{uuid.uuid4().hex}")
        chat = Chat(
            chat_id=cid,
        ...
        with self._connect() as db:
            db.execute(
                """
                INSERT INTO chats (chat_id, title, created_at, updated_at, persona_id, archived)
                VALUES (?, ?, ?, ?, ?, 0)
                """,
    ...
    def add_message(
        ...
        message = ChatMessage(
            message_id=str(message_id or f"msg_{uuid.uuid4().hex}"),
            chat_id=str(chat_id),
            role=str(role),
            text=str(text),
        ...
        with self._connect() as db:
            db.execute(
                """
                INSERT INTO messages (message_id, chat_id, role, text, metadata_json, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    message.message_id,
    ...
    def list_messages(self, chat_id: str, *, limit: int = 10000) -> list[ChatMessage]:
        with self._connect() as db:
            rows = db.execute(
                """
                SELECT message_id, chat_id, role, text, metadata_json, created_at
                FROM messages
                WHERE chat_id = ?
    ...
    def archive_chat(self, chat_id: str, archived: bool = True) -> None:
        with self._connect() as db:
            db.execute(
                "UPDATE chats SET archived = ?, updated_at = ? WHERE chat_id = ?",
                (1 if archived else 0, time.time(), str(chat_id)),
            )
    ...
    def archive_chats_except(self, keep_ids: set[str]) -> None:
        ...
        with self._connect() as db:
            ...
            db.execute(
                f"UPDATE chats SET archived = 1, updated_at = ? WHERE chat_id NOT IN ({placeholders})",
                (time.time(), *sorted(keep)),
            )
    ...
    def _row_to_chat(row: sqlite3.Row) -> Chat:
        return Chat(
            ...
            persona_id=str(row["persona_id"] or "default"),
