...
def is_ollama_alive(base_url: str = "http://127.0.0.1:11434", timeout: float = 0.5) -> bool:
    try:
        with urllib_request.urlopen(_clean_base_url(base_url) + "/api/tags", timeout=timeout) as resp:
...
def _build_env(models_dir: str = "") -> dict[str, str]:
    env = dict(os.environ)
    clean = str(models_dir or "").strip().strip('"')
    if clean:
        env["OLLAMA_MODELS"] = str(Path(clean).expanduser())
    return env


def _serve_exe_or_default(serve_exe: str = "") -> str:
    return str(serve_exe or "").strip().strip('"') or shutil.which("ollama") or "ollama"
...
def _popen_serve(serve_exe: str, models_dir: str) -> tuple[bool, str]:
    ...
    if _OWNED_OLLAMA_PROCESS is not None and _OWNED_OLLAMA_PROCESS.poll() is None:
        return True, "Ollama serve process already owned by MMis UI"
    ...
    try:
        ...
        return False, f"ollama exe not found: {exe}"
...
def _run_ollama_list(models_dir: str, timeout: float = 8.0) -> tuple[bool, str]:
    ...
    try:
        ...
        return (True, "ollama list completed") if result.returncode == 0 else (False, f"ollama list exited with code {result.returncode}")
...
def ensure_ollama_started(
    *,
    ...
    with _START_LOCK:
        if is_ollama_alive(base_url, timeout=0.5):
            return True, "Ollama API already running"
        if (now - _LAST_START_ATTEMPT_AT) < _START_COOLDOWN_SEC and _LAST_START_MODE == mode and not force:
            return False, "Ollama start already attempted recently"
        _LAST_START_ATTEMPT_AT = now
        _LAST_START_MODE = mode
        ok, message = _run_ollama_list(models_dir=models_dir, timeout=wait_sec) if mode == "ui" else _popen_serve(serve_exe=serve_exe, models_dir=models_dir)
        if not ok:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.35)
    return False, f"{message}; /api/tags is still unavailable"
