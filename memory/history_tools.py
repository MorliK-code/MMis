...
В отличие от memory_retrieve (векторный поиск), эти инструменты
дают точный доступ к сообщениям по порядку, диапазону или поиску.
"""
...
def history_read_recent_tool_spec() -> ToolSpec:
    ...
    - "последние 10 моих сообщений"
    - "что я писал до этого"
    - "последние ответы ассистента"
    """
    return ToolSpec(
...
def history_read_range_tool_spec() -> ToolSpec:
    ...
    Используется для запросов типа:
    - "все мои сообщения за сегодня"
    - "что я писал вчера вечером"
    - "диалог за 12 марта"
    """
    return ToolSpec(
...
def history_search_tool_spec() -> ToolSpec:
    ...
    Используется для запросов типа:
    - "я вообще когда-то говорил про Таню?"
    - "найди где я писал про summary"
    - "упоминал ли я Python 3.11?"
    """
    return ToolSpec(
        name="history_search",
